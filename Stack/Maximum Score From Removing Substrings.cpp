...
public class Solution {
    ...
    private string RemoveSubstring(string s, string matchStr) {
        ...
        foreach (char ch in s) {
            if (stack.Count > 0 && ch == matchStr[1] && stack.Peek() == matchStr[0]) {
                stack.Pop();
            } else {
                stack.Push(ch);
            }
        ...
        char[] remainArr = stack.ToArray();
        Array.Reverse(remainArr);
        return new string(remainArr);
    }
}
...
public class Solution {
    ...
    private string RemoveSubstring(string inputString, string matchStr) {
        ...
        for (int i = 0; i < inputString.Length; i++) {
            ...
            if (j > 1 && sb[j - 2] == matchStr[0] && sb[j - 1] == matchStr[1]) {
                sb.Remove(j - 2, 2);
                j -= 2;
            }
        }
