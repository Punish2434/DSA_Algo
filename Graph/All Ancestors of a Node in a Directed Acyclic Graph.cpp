...
public class Solution {
    public IList<IList<int>> GetAncestors(int n, int[][] edges) {
        ...
        var ancestorSets = new HashSet<int>[n];
        for (int i = 0; i < n; i++) {
            ancestorSets[i] = new HashSet<int>();
        }
        ...
        while (queue.Count > 0) {
            ...
            foreach (var neighbor in adjList[node]) {
                ancestorSets[neighbor].Add(node);
                ancestorSets[neighbor].UnionWith(ancestorSets[node]);
                inDegree[neighbor]--;
        ...
        foreach (var ancestors in ancestorSets) {
            result.Add(ancestors.OrderBy(a => a).ToList());
        }
...
public class Program {
    public static void Main() {
        ...
        var edges1 = new int[][] {
            new int[] {0, 3}, new int[] {0, 4}, new int[] {1, 3}, new int[] {2, 4}, new int[] {2, 7}, 
            new int[] {3, 5}, new int[] {3, 6}, new int[] {3, 7}, new int[] {4, 6}
        };
        var result1 = solution.GetAncestors(8, edges1);
        foreach (var ancestors in result1) {
            Console.WriteLine(string.Join(", ", ancestors));
        }
        
        var edges2 = new int[][] {
            new int[] {0, 1}, new int[] {0, 2}, new int[] {0, 3}, new int[] {0, 4}, new int[] {1, 2}, 
            new int[] {1, 3}, new int[] {1, 4}, new int[] {2, 3}, new int[] {2, 4}, new int[] {3, 4}
        };
        var result2 = solution.GetAncestors(5, edges2);
        foreach (var ancestors in result2) {
            Console.WriteLine(string.Join(", ", ancestors));
        }
    }
}
...
// Space Complexity: O(n + e) for the adjacency list and the ancestors sets.
