...
public class Solution {
    ...
    private void Solve(TreeNode root, string curr) {
        if (root == null) {
            return;
        }
        ...
        if (root.left == null && root.right == null) {
            if (string.IsNullOrEmpty(result) || string.CompareOrdinal(result, curr) > 0) {
                result = curr;
            }
            return;
        }
...
// - We use `string.IsNullOrEmpty(result)` instead of `result.equals("")`.
// - We use `string.CompareOrdinal(result, curr)` instead of `result.compareTo(curr)` to compare strings.
...
public class Solution {
    public string SmallestFromLeaf(TreeNode root) {
        ...
        string result = "";
        queue.Enqueue((root, Convert.ToString((char)(root.val + 'a'))));
        
        while (queue.Count > 0) {
            (TreeNode node, string curr) = queue.Dequeue();
            ...
            if (node.left != null) {
                queue.Enqueue((node.left, Convert.ToString((char)(node.left.val + 'a')) + curr));
            }
            
            if (node.right != null) {
                queue.Enqueue((node.right, Convert.ToString((char)(node.right.val + 'a')) + curr));
            }
        }
...
// - We use `Queue<(TreeNode, string)>` instead of `Queue<Pair<TreeNode, String>>`.
// - We use `string.IsNullOrEmpty(result)` instead of `result.equals("")`.
// - We use `string.CompareOrdinal(result, curr)` instead of `result.compareTo(curr)` to compare strings.
