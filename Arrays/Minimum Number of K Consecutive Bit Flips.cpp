...
public class Solution {
    public int MinKBitFlips(int[] nums, int k) {
        int n = nums.Length;
        int flips = 0;
        Deque<int> flipQueue = new Deque<int>();
        int flipCountFromPastForCurri = 0;

        for (int i = 0; i < n; i++) {
            if (i >= k) {
                flipCountFromPastForCurri -= flipQueue.Dequeue();
            }
...
// Custom Deque implementation since C# does not have a built-in Deque class
public class Deque<T> {
    ...
    public T Dequeue() {
        if (list.Count == 0) throw new InvalidOperationException("The deque is empty.");
