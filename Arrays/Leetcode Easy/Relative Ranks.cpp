...
public class Solution {
    public string[] FindRelativeRanks(int[] score) {
        ...
        for (int i = 0; i < n; i++) {
            pq.Enqueue(new int[] { score[i], i });
        }
        
        int rank = 1;
        
        while (pq.Count > 0) {
            int[] athlete = pq.Dequeue();
            int idx = athlete[1];
